...
for(j in 1:length(nbasisv)){
  ...
  rhs <- paste(alphas0,forcingstxt,splines,alphas1,sep = '+')
  ...
  names(fixed.prior.mean)[nelements+seq(1,3)] <- c('Ss','Vs','Cs')
  names(fixed.prior.sd)[nelements+seq(1,3)] <- c('Ss','Vs','Cs')
...
save.image(file = paste0('./results/results_new_',dataset,'_',method,'_',nbasisv,'_Splines_mixed.RData'))
